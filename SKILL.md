...
ImageMagick processing MCP server. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `imagemagick` or user asks by name
...
2. **Confirm tooling** - Verify `imagemagick` runtime/CLI/MCP is available; document install if missing.
